    ...
    if current_time - last_check_time >= 15:
        for elm in safety :
            if elm not in classes : 
                now = time.localtime()
                filename = f"save{now.tm_year}{now.tm_mon}{now.tm_mday}_{now.tm_hour}{now.tm_min}{now.tm_sec}.avi"
                out = cv2.VideoWriter(filename,cv2.VideoWriter_fourcc('M','J','P','G'), 10, (768,567))
